...
<table width="900px" height="0px" cellpadding="10" cellspacing="0" bgcolor="#FFFFFF"
 ...
 <tr><td valign="top" height="10%"  align="center">Neue Gruppe: 
 <input type="text" name="gruppen_name" size="40" value="<?php echo $_POST['gruppen_name']; ?>">&nbsp;&nbsp;
...
$gruppen_name=htmlentities($_POST['gruppen_name']);
...
<input type="radio" name="mehrstellig" value="2" checked> 1stellige PLZ's &nbsp;&nbsp;&nbsp;
<input type="radio" name="mehrstellig" value="2"> 2stellige PLZ's &nbsp;&nbsp;&nbsp;
<input type="radio" name="mehrstellig" value="3"> 3stellige PLZ's &nbsp;&nbsp;&nbsp;
<input type="radio" name="mehrstellig" value="4"> 4stellige PLZ's &nbsp;&nbsp;&nbsp;
<br>
...
?>
            ...
            <tr>
                 ...
                 <tr><td colspan="3">
                 <?php $gruppen_suche=mysqli_query($link,"select * from gruppe_definition order by gr_def_id desc");
                 ...
                 <td>
                 <input type="submit" value="in Gruppe speichern" name="liste_speichern" tabindex="15"
                 ...
                 $select_doppelte=mysqli_query($link," select name, vorname, email from gruppen where email = '$vorauswahl_result[email]' and gruppe = 
                 '$_POST[gruppen_auswahl]' ");
                 while($result_doppelte = mysqli_fetch_array($select_doppelte, MYSQLI_BOTH ) ) {
                 $doppelte_name = $result_doppelte['name'];
                 $doppelte_vorname = $result_doppelte['vorname'];
                 $doppelte = $result_doppelte['email'];
                 }
                 
                 if (
                    (   $doppelte != $vorauswahl_result['email'] ) ||
                    ( ( $doppelte == $vorauswahl_result['email'] ) && ( $doppelte_vorname != $vorauswahl_result['vorname'] ) )
                    ) {
                 ...
                 $gruppen_suche_2=mysqli_query($link, "select * from gruppe_definition order by gr_def_id desc" );
      ...
      // Neue Adressen markieren
      $zeit_select=mysqli_query($link,"select zeitstempel, ausbildung, telefon, mobil_telefon from adressen where kd_nr = '$result_gruppe[kd_nr]' ");
      while ( $zeit_result = mysqli_fetch_array ( $zeit_select, MYSQLI_BOTH ) ) {
      
      $ausbildung=$zeit_result['ausbildung'];
      $telefon=$zeit_result['telefon'];
      $mobil_telefon=$zeit_result['mobil_telefon'];
      
      if ( ($zeit_result['zeitstempel'] + 432000) >= time() )  { $neukunde = "neu"; }
      else { $neukunde = ""; }
      }
      
      $email = $result_gruppe[email];
      if ( $result_gruppe[email] == "" ) { $email = $mobil_telefon."<br>".$telefon; }
      $teilnehmer.="<tr><td><t1>$result_gruppe[gruppen_id]</t1></td><td><t1>&nbsp;&nbsp;$result_gruppe[name]</t1></td><td><t1>&nbsp;&nbsp;$result_gruppe[vorname]</t1></td><td><t1>&nbsp;&nbsp;$email</t1></td><td><t1>&nbsp;&nbsp;$ausbildung</t1></td><td><t1>&nbsp;&nbsp;$neukunde</t1></td></tr>";
      }
      echo "
      <tr><td colspan=\"3\">
      <table><tr>
      <td><t1>ID<t1></td><td><t1>&nbsp;&nbsp;Name</t1></td><td><t1>&nbsp;&nbsp;Vorname</t1></td><td><t1>&nbsp;&nbsp;eMail</t1></td><td><t1>&nbsp;&nbsp;Ausbildung</t1></td><td><t1>&nbsp;&nbsp;NEU</t1></td></tr>
      ...
      if ( $_POST['teilnehmerloeschen'] == TRUE )  { mysqli_query($link,"delete from gruppen where gruppen_id = '$_POST[id_loeschen]' " ); }
       ...
       ?>
                 ...
                 <tr><td colspan="3">
                 <?php $gruppen_suche_vc=mysqli_query($link,"select * from gruppe_definition order by gr_def_id desc");
                       while($gruppen_result_vc=mysqli_fetch_array($gruppen_suche_vc, MYSQLI_BOTH )) {
                       if ( $gruppen_result_vc['gr_def_id'] == $_POST['gruppe_vcard'] )  { $selected_vc = "selected"; }
                       $gruppe_vcard.="<option $selected_vc value=\"$gruppen_result_vc[gr_def_id]\"> $gruppen_result_vc[gruppe_name]</option>";
                       $selected_vc="";
                       }
                 ?>
                 <t1>Gruppe ausw&auml;hlen und als V-Card-Datensatz exportieren<br>
                 </td></tr>
                 <tr><td valign="middle">
                 <select name="gruppe_vcard">
                 <?php echo $gruppe_vcard; ?>
                 </select>&nbsp;&nbsp;<br>
                 </td>
                 <td  valign="middle"><t1>
                  &nbsp;                
                 ...
                 <td>
                 <input type="submit" value="als V-Card exportieren" name="vcard_speichern" tabindex="15"
                  style=" width: 200px; height: 30px; color: #000000; font-size: 18px; background-color: orange;  border: 0px; border-radius: 5px 5px 5px 5px;">
...
<?php 
     ...
     if ( $_POST['vcard_speichern'] == TRUE )  {
     if(!is_dir("C:/vcard_ordner"))
{mkdir("C:/vcard_ordner");}
     $handle = fopen("C:/vcard_ordner/vcard.txt", "x+");
      $select_gruppe_vc=mysqli_query($link, "select * from gruppen where gruppe = '$_POST[gruppe_vcard]' " );  
      while ( $result_gruppe_vc = mysqli_fetch_array( $select_gruppe_vc, MYSQLI_BOTH ) )  {
      
      $file.="
BEGIN:VCARD\n
VERSION:3.0\n
N:$result_gruppe_vc[name];$result_gruppe_vc[vorname];;;\n
FN:$result_gruppe_vc[vorname] $result_gruppe_vc[name]\n
EMAIL;TYPE=INTERNET;TYPE=HOME:$result_gruppe_vc[email]\n
END:VCARD\n";
      ...
      fclose($handle);
                 ...
                 $gruppen_suche_1=mysqli_query($link, "select * from gruppe_definition order by gr_def_id desc" );
                 ...
                 <td><t1>
                  <input type="submit" value="Gruppe l&ouml;schen ?" name="gruppenloesch" tabindex="18"
                 ...
                 <?php 
                      if ( $_POST['gruppenloesch'] == TRUE )  { echo "
                      
                   </td><td colspan=\"3\" align=\"center\"><t1><br>
                  <input type=\"submit\" value=\"Gruppe l&ouml;schen !\" name=\"gruppeloeschen\" tabindex=\"19\"
