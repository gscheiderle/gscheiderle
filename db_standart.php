...
$text=mysql_query("select * from seitentext where seiten_id = '$_GET[seitenid]'");
while ($seitentext = mysql_fetch_array($text)){
$headline=$seitentext['ueberschrift'];
$headline_2=$seitentext['ueberschrift_2'];
$fliesstext=$seitentext['text'];}

$xx=1;
$bilder=mysql_query("select * from seitenbilder where seiten_id = '$_GET[seitenid]'");
while ($seitenbild = mysql_fetch_array($bilder)){
...
$bild_db=mysql_query("select bild_breite, bild_hoehe, rahmen_breite, rahmen_hoehe, speicherort from bildspeicher
where logo_id = '$seitenbild[bild_id]'");
while($bild_ort=mysql_fetch_array($bild_db)){
